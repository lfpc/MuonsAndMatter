...
def run_fem(magn_params:dict,
   ...
   # make the mesh
   gmsh_model, term_1, term_2 = roxie_evaluator.make_SHIP_iron_core( magn_params['Xmgap1(m)'],
                                                                     magn_params['Xcore1(m)'],
                                                                     magn_params['Xvoid1(m)'],
                                                                     magn_params['Xyoke1(m)'],
                                                                     magn_params['Xmgap2(m)'],
   ...
   # Make the solver
   solver = roxie_evaluator.PoissonSolver(gmsh_model.mesh, [mu_iron, mu_air], (term_1, term_2),
                                          max_iter=max_iter, quad_order=element_order+1)
   
   # solve the problem
   start = time.time()
   x = solver.solve(NI, tolerance=tolerance, use_cg=True)
...
def run(magn_params:dict,
    ...
    for i in range(n_magnets):
        ...
        B += get_grid_data(**run_fem(params, (0.5,0.5,delta_z)), new_points=points)[1]
    ...
    if save_results:
        with gzip.open(os.path.join(output_directory,'results_nc.pkl'), 'wb') as f:
            pickle.dump({'points':points, 'B':B}, f)
        print('Results saved to', os.path.join(output_directory,'results_nc.pkl'))
    return {'points':points, 'B':B}
...
if __name__ == '__main__':
   ...
   magn_params = {k:v[0] for k,v in magn_params.items()}
   t1 = time.time()
   d = run(magn_params,output_directory, apply_grid=True, apply_symmetry=True, plot_results=False, save_results=False)
