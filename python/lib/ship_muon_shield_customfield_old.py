...
def design_muon_shield(params,fSC_mag = True, simulate_fields = False, field_map_file = None, cores_field:int = 1,extra_magnet = False):
    ...
    if field_map_file is not None or simulate_fields: 
        ...
        max_x = np.round(max(np.max(dXIn + dXIn * ratio_yokes + gapIn+midGapIn), np.max(dXOut + dXOut * ratio_yokes+gapOut+midGapOut))/100,decimals=1).item()
        max_y = np.round(max(np.max(dYIn + dXIn * ratio_yokes), np.max(dYOut + dXOut * ratio_yokes))/100,decimals=1).item()
        d_space = (max_x+0.5, max_y+0.5, (-1, np.ceil((Z[-1]+dZf[-1]+50+10)/100).item()))
        resol = magnet_simulations.RESOL_DEF
        #assert d_space[0] % resol[0] == 0, f"{d_space[0]}, {resol[0]}, {d_space[0] % resol[0]}"
        #assert d_space[1] % resol[1] == 0, f"{d_space[1]}, {resol[1]}, {d_space[1] % resol[1]}"
        #assert (d_space[2][1]-d_space[2][0]) % resol[2] == 0, f"{d_space[2]}, {resol[2]}, {(d_space[2][1]-d_space[2][0]) % resol[2]}"
